...
export async function GET(){
  ...
  try{
    await ensureSchema();
    return Response.json({ok:true,storage:uploadRoot(),email:'configured',authentication:{mode,accountSso:studioSsoConfigured()?'configured':'unavailable',sessionVersion:2,navigation:'fresh-request'},workflowVersion:7});
  }catch(error){
