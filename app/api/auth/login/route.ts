...
function failure(request:Request,returnTo:string){return wantsJson(request)?Response.json({error:'That email or password was not recognized.'},{status:401,headers:{'cache-control':'no-store'}}):redirectWith(request,`/login?error=invalid&returnTo=${encodeURIComponent(returnTo)}`);}
...
export async function POST(request: Request) {
  ...
  if (isStudioAdmin(email)) {
    ...
    if (account && verifyPassword(password, account.password_hash)) user = {userId: account.id, email: account.email, displayName: account.display_name, fullName: account.display_name, role: 'client'};
  }
