...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  for(const file of files){
    ...
    if(!matchesKind)return Response.json({error:`Unsupported file for ${kind}: ${file.name}`},{status:400});
    ...
    try{await addAsset(user,id,{kind,objectKey:key,filename:file.name.slice(0,180),mimeType:file.type,byteSize:file.size,label});count++;}
    catch(error){await deleteStoredFile(key);throw error;}
  }
