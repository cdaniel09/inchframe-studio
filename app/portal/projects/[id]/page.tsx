...
import { getProjectBundle,isStudioAdmin } from '@/lib/data';
...
async function ProjectContent({params}:{params:Promise<{id:string}>}) {
  ...
  const canDecide=bundle.project.owner_id===user.userId;
  ...
  return <main className="portal-page"><PortalHeader user={user}/><section className="project-banner"><div className="shell"><a className="back-link" href="/portal">← All projects</a><div className="project-title-grid"><div><p className="eyebrow"><span>●</span> {titleCase(bundle.project.project_type)}</p><h1>{bundle.project.title}</h1><p>{bundle.project.brief}</p></div><div className="project-facts"><div><span>Status</span><strong>{titleCase(bundle.project.status)}</strong></div><div><span>Target date</span><strong>{shortDate(bundle.project.due_date)}</strong></div><div><span>Budget</span><strong>{titleCase(bundle.project.budget_range)||'To discuss'}</strong></div></div></div></div></section><section className="project-content"><div className="shell project-layout"><aside><nav className="project-nav"><a href="#overview">Overview</a>{showProduction&&<><a href="#advanced">Advanced brief</a><a href="#seeds">Seed library <b>{seeds.length}</b></a><a href="#audio">Audio <b>{audio.length}</b></a><a href="#review">Review queue <b>{reviews.length}</b></a><a href="#delivery">Delivery <b>{deliveries.length}</b></a></>}</nav><div className="brief-card"><span>Client</span><strong>{bundle.project.owner_email}</strong><span>Audience</span><p>{bundle.project.audience||'Not specified'}</p><span>Platforms</span><p>{bundle.project.platforms||'Not specified'}</p><span>Budget</span><p>{titleCase(bundle.project.budget_range)||'Not specified'}</p>{showProduction&&<><span>Formats</span><p>{formats}</p><span>Style direction</span><p>{bundle.project.style_notes||'Not specified'}</p></>}</div></aside><div className="project-sections"><section id="overview" className="portal-card"><div className="card-heading"><div><span>PROJECT GATE</span><h2>{admin?'Review this inquiry':'What happens next'}</h2></div></div>{admin?<AdminInquiryControls projectId={id} status={bundle.project.status}/>:<ClientGate projectId={id} status={bundle.project.status} unlocked={clientUnlocked}/>}</section>{showProduction&&<section id="advanced" className="portal-card"><div className="card-heading"><div><span>ADVANCED INTAKE</span><h2>Production details</h2></div></div>{admin?<AdvancedSummary project={bundle.project}/>:<AdvancedIntakeForm projectId={id} project={advancedFormProject}/>}</section>}{showProduction&&<section className="portal-card"><div className="card-heading"><div><span>UPLOADS</span><h2>{admin?'Add work for review':'Add images and audio'}</h2></div></div><UploadPanel projectId={id} isAdmin={admin}/></section>}{showProduction&&<><AssetSection title="Image seeds" id="seeds" empty="No seed images have been added yet." assets={seeds} comments={bundle.comments} projectId={id} canDecide={false}/><AssetSection title="Reference audio" id="audio" empty="No audio references have been added yet." assets={audio} comments={bundle.comments} projectId={id} canDecide={false}/><AssetSection title="Review queue" id="review" empty={admin?'Upload a review version when it is ready for the client.':'Nothing needs your review right now.'} assets={reviews} comments={bundle.comments} projectId={id} canDecide={canDecide}/><AssetSection title="Final delivery" id="delivery" empty="Approved delivery files will appear here." assets={deliveries} comments={bundle.comments} projectId={id} canDecide={canDecide}/></>}</div></div></section></main>;
}
...
function ClientGate({projectId,status,unlocked}:{projectId:string;status:string;unlocked:boolean}) {
  ...
  return <div className="stage-message"><strong>Inquiry received.</strong><p>The Studio will review fit, schedule, and budget. If accepted, you’ll receive an email with a one-time project code.</p></div>;
}
