...
export function CreatorApplicationForm({profile}:{profile:CreatorProfile|null}) {
  ...
  return <form className="intake-form creator-application" onSubmit={submit}>
    ...
    <div className="form-grid">
      <label className="wide"><span>Profile icon {profile?.avatar_object_key?'(replace only if needed)':'(required)'}</span><input name="avatar" type="file" accept="image/jpeg,image/png,image/webp" required={!profile?.avatar_object_key}/><small>JPG, PNG, or WebP · 3 MB maximum.</small></label>
